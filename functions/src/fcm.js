...
exports.sendFcm = functions.region('europe-west3').https.onCall(async(data,context) => {
    ...
    const batchResponce = await messaging.sendMulticast(fcmMessage);
    const failedTokens = [];

    if (batchResponce.failureCount > 0) {
        batchResponce.responses.forEach((resp, idx) => {
            if (!resp.success) {
...
function checkIfAllowed(context, chatAdmins){
 ...
 function transformToArr(snapVal) {
  ...
  async function getUserTokens(uid) {
    ...
    const userTokensSnap = database.ref('/fcm_tokens').orderByValue().equalTo(uid).once('value');
